...
def run_one_epoch(epoch, student, optimizer_student, cvae_student, optimizer_cvae_student, teacher, cvae_teacher, data_loader, epoch_step_num, training=True):
    ...
    with tqdm.tqdm(data_loader, total=epoch_step_num, ncols=0) as pbar:
        for n_iter, (clips, targets, scores, ssl_clips, ssl_targets, flags) in enumerate(pbar):
            ...
            attention_4f_teacher = attention_4f_teacher.unsqueeze(-1).unsqueeze(-1)
    ...
    with tqdm.tqdm(data_loader, total=epoch_step_num, ncols=0) as pbar:
        for n_iter, (clips, targets, scores, ssl_clips, ssl_targets, flags) in enumerate(pbar):
            ...
            if flags[0]:
                ...
                ssl_attention_5c, ssl_attention_4f = student(ssl_video_feature, mode = 'att')
...
if __name__ == '__main__':
    ...
    # cvae_teacher.load_state_dict(torch.load())
    ...
    Setup optimizer
    """
    optimizer_student = torch.optim.Adam(net_student.parameters(),
                                         lr=learning_rate,
                                         betas=(0.9, 0.999),
    ...
    Start training
    """
    start_epoch = resume_training(resume, net_student, optimizer_student)
