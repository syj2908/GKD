...
from GAF.thumos14.BDNet_student import BDNet_student as BDNet, CVAE_student as CVAE
...
def save_model(epoch, model, optimizer, cvae, optimizer_cvae):
    ...
    torch.save(cvae.module.state_dict(), os.path.join(
        checkpoint_path, 'checkpoint-{}_cvae.ckpt'.format(epoch)))
...
def resume_training(resume, model, optimizer, cvae, optimizer_cvae):
    ...
    if resume > 0:
        ...
        cvae_path = os.path.join(
            checkpoint_path, 'checkpoint-{}_cvae.ckpt'.format(resume))
...
def calc_bce_loss(start, end, scores):
    ...
    loss_start = F.binary_cross_entropy(
        start.view(-1), scores[:, 0].contiguous().view(-1).cuda(), reduction='mean')
    loss_end = F.binary_cross_entropy(
        end.view(-1), scores[:, 1].contiguous().view(-1).cuda(), reduction='mean')
