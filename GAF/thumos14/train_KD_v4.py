    ...
    with tqdm.tqdm(data_loader, total=epoch_step_num, ncols=0) as pbar:
        for n_iter, (clips, targets, scores, ssl_clips, ssl_targets, flags) in enumerate(pbar):
            ...
            means_5c, log_var_5c, z_5c, recon_feature_5c_teacher = cvae_teacher(
                'forward', video_feature['Mixed_5c'], attention_5c_teacher)
            means_4f, log_var_4f, z_4f, recon_feature_4f_teacher = cvae_teacher(
                'forward', video_feature['Mixed_4f'], attention_4f_teacher)
            ...
            ).numpy())-float(KD_loss_cvae.cpu().detach().numpy())), KD_loss='{:.5f}'.format(float(KD_loss_cvae.cpu().detach().numpy())))

    loss_cvae_val /= (n_iter + 1)
    loss_cvae_KD_val /= (n_iter + 1)

    plog = 'CVAE total - {:.5f}, KD - {:.5f}'.format(
        loss_cvae_val, loss_cvae_KD_val)
    print(plog)

    """
    stage 2: update backbone
    """
    for n, m in student._modules.items():
...
if __name__ == '__main__':
    ...
    dict_trained = torch.load(pretrained_backbone)
