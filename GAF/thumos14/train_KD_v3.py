...
from GAF.thumos14.BDNet_teacher import BDNet_teacher, CVAE_teacher
from GAF.thumos14.BDNet_student import BDNet_student, CVAE_student
...
checkpoint_path = config['training']['checkpoint_path_v3']
pretrained_path = config['training']['pretrained_path_new']
pretrained_path_cvae = config['training']['pretrained_path_cvae_new']
...
def run_one_epoch(epoch, student, optimizer_student, cvae_student, optimizer_cvae_student, teacher, cvae_teacher, data_loader, epoch_step_num, training=True):
    ...
    with tqdm.tqdm(data_loader, total=epoch_step_num, ncols=0) as pbar:
        for n_iter, (clips, targets, scores, ssl_clips, ssl_targets, flags) in enumerate(pbar):
            ...
            pbar.set_postfix(loss='{:.5f}'.format(float(loss.cpu().detach(
            ).numpy())), KD_loss='{:.5f}'.format(float(KD_loss_cvae.cpu().detach().numpy())))

    loss_cvae_val /= (n_iter + 1)
    loss_cvae_KD_val /= (n_iter + 1)

    plog = 'CVAE - {:.5f}, KD - {:.5f}'.format(loss_cvae_val, loss_cvae_KD_val)
    print(plog)

    """
    stage 2: update detector
    """
    for name, param in student.named_parameters():
        param.requires_grad = True
        param.grad = None
    ...
    with tqdm.tqdm(data_loader, total=epoch_step_num, ncols=0) as pbar:
        for n_iter, (clips, targets, scores, ssl_clips, ssl_targets, flags) in enumerate(pbar):
            ...
            cost += KD_loss * beta * 10
            writer.add_scalar('loss/loss_cost_total', cost, GLOBAL_N)
...
if __name__ == '__main__':
    ...
    """
    setup student model and load pretrained weight
    """
    net_student = BDNet_student(in_channels=config['model']['in_channels'],
                                backbone_model=config['model']['backbone_model'])
    # load pretrained weight
    dict_trained = torch.load(pretrained_weight)
    dict_new = net_student.state_dict().copy()
    new_list = list(net_student.state_dict().keys())
    trained_list = list(dict_trained.keys())
    for i in range(442):
        dict_new[new_list[i]] = dict_trained[trained_list[i]]
    net_student.load_state_dict(dict_new)
    print("pretrained Top-AFSD weight loaded")
    # # freeze backbone
    # for name, module in net_student._modules.items():
    #     if name == "backbone":
    #         for p in module.parameters():
    #             p.requires_grad = False
    # print("backbone weight freezed")
